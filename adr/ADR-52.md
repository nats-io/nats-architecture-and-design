...
Often the only part of a message users care for is the, a good example is counters introduced in ADR-49 where there is a data section in the body and a control section in the headers. For clients that only care for the current count there is no need to even download the headers from the server.
...
type JSApiMsgGetRequest struct {
	...
	NoHeaders bool `no_hdr,omitempty
}`
```
