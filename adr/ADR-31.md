...
If the `no_hdr` option is set for any of these kinds of requests the messages being delivered will have no headers including the headers like `Nats-Sequence` will be unset and the final message holding headers like `Nats-Num-Pending` will still be delivered as specified above.
