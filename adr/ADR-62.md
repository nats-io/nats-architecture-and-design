...
| **Evacuate a stream/consumer peer** | `$JS.API.STREAM.PEER.EVACUATE.<stream>` (stream)<br/>`$JS.API.CONSUMER.PEER.EVACUATE.<stream>.<consumer>` (consumer) | - Marks the peer as evacuating, and assigns a new peer to catch up. If no replacement peer is available, the request is rejected.<br/>- The evacuating peer can still help to catch up the new peer before being removed, for example when evacuating the last peer for a R1 stream/consumer. |
| **Remove a stream/consumer peer** | `$JS.API.STREAM.PEER.REMOVE.<stream>` (stream)<br/>`$JS.API.CONSUMER.PEER.REMOVE.<stream>.<consumer>` (consumer) | - Removes the peer from the stream/consumer immediately. If no replacement peer is available, the request is rejected.<br/>- Should only be used when the peer being removed can be replaced by another, the peer is meant to come back (if temporarily offline), and needs to remain part of the meta group. If the peer is decommissioned or is otherwise not meant to come back, see "Remove a server peer" instead, which removes the peer from the cluster without rejecting even if there are no replacement peers available (may leave streams/consumers under-replicated). |
| **Evacuate a server peer** | `$JS.API.SERVER.EVACUATE`<br/>(system account, evacuates all assets from a server) | - All streams and consumers on this peer are evacuated off it.<br/>- A replacement peer is selected, if available, and the streams/consumers move accordingly.<br/>- If no new peer is available the group will run under-replicated (if R>1) until a new peer is added to the cluster that satisfies the stream placement, at which point it's automatically added to the stream/consumer peer set.<br/>- If no new peer is available and the stream/consumer is already at R1, it will remain and not be evacuated. Peer-removing the server at this point will also not move the stream/consumer if no new peer can be selected. If evacuation is not able to move off some streams/consumers due to them being R1 with no replacement peers available, add a new replacement node first and retry evacuating. |
| **Remove a server peer** | `$JS.API.SERVER.REMOVE`<br/>(system account, peer-remove a server and remove from all assets) | - Removes the peer from all streams and consumers immediately, and assigns a new peer if available.<br/>- If no new peer is available the group will run under-replicated (if R>1) until a new peer is added to the cluster that satisfies the stream placement, at which point it's automatically added to the stream/consumer peer set.<br/>- Peer-removing the last peer, for example if R1, of a stream/consumer is allowed but moves it to a new peer without preserving the data (since the last copy is removed). Prefer evacuating a stream/consumer peer first if it's still able to catch up other peers.<br/>- Removing a peer is a valid strategy to get out of "no quorum" when no leader can be elected if a quorum of peers can't come back. Make sure to scale down the stream/consumer to the appropriate replica count afterward. For example, peer-removing 2 dead nodes such that it runs as a R1 stream/consumer, you can then scale down the stream/consumer config to R1 as well such that it matches its peer set. Be careful though with peer-removing liberally when peers are planned to come back, if a peer can't be replaced its slot will remain empty, so when the peer does come back it does not get re-added to the peer set (since it was peer-removed), scale down the stream/consumer, wait for it to settle (to ensure re-adding the same peer is allowed), then scale it back up. |
