...
| 8        | 2026-05-22 | @MauriceVanVeen             | Document `DiscardNew` interaction                                                          | TBD            |
...
| `Nats-Schedule-Source`    | Instructs the schedule to read the last message on the given subject and publish it to the target. If no message exists on the source subject, the schedule's own body and headers is published as a fallback (a second fallback trigger exists under `DiscardNew`, see [Discard Policy Interaction](#discard-policy-interaction)). Wildcards are not supported.                                                       |
...
* `DiscardNew` is supported and protects stored schedules from `DiscardOld` eviction, but affects how a full stream interacts with schedules, see [Discard Policy Interaction](#discard-policy-interaction).
...
| **Stream limits on schedule lifetime**        | `MaxAge` shorter than the firing interval deletes the schedule before it fires, prefer `Nats-TTL` on the schedule itself. Schedule removal due to `DiscardOld`, `MaxMsgs`, `MaxBytes`, etc. similarly removes and disables the schedule. `DiscardNew` instead preserves the schedule against these count/byte limits, but `MaxAge` always expires it regardless of discard policy, see [Discard Policy Interaction](#discard-policy-interaction). | Same as `Limits`, plus the `WorkQueue` caveats above.                                                                                | Same as `Limits`, plus the `Interest` caveats above.                                                               |
...
#### Discard Policy Interaction

The discard policy is orthogonal to the retention policy above. The retention table covers the eviction-based limits (`DiscardOld`, `MaxAge`, `MaxMsgs`, `MaxBytes`), all of which can remove a stored schedule and disable it. `DiscardNew` is the opposite mechanism: instead of evicting an existing message to make room, it rejects the incoming write. As a result, a schedule message, once stored, is **protected** from `MaxMsgs`/`MaxBytes` eviction under `DiscardNew`. `MaxAge` is unaffected by the discard policy and always expires the schedule by age, so the `Nats-TTL` recommendation above still applies.

Two distinct writes are affected when the stream is full (`MaxMsgs`/`MaxBytes`, or `MaxMsgsPerSubject` under `DiscardNewPerSubject`):

- **Storing the schedule.** Publishing a new schedule onto a fresh subject is a net-new message and is rejected when the stream is full, the same as any other publish. Updating an existing schedule on the same subject is a rollup (`Nats-Rollup: sub`) that replaces the prior message rather than growing the count. Under `MaxMsgs` this is net-zero on message count and so reliably succeeds even at the limit. Under `MaxBytes` the purge frees the old message's bytes, unless the new schedule is larger such that the result would exceed `MaxBytes`, in which case it is rejected. Both `MaxMsgs` and `MaxBytes` are absolute limits that are not allowed to be exceeded.

- **Firing the schedule.** The behavior depends on whether the scheduled message grows the stream:
  - **Single delayed publish (`@at`)**: the delayed message purges the schedule message and stores the delayed message, a "replace" that frees one slot before storing one back. This **bypasses** the `DiscardNew` limit so the delayed message is always persisted and is never lost merely because the stream was full. The bypass is safe because the delayed message is guaranteed to be no larger than the schedule it replaces,so the replace can never push the stream over `MaxBytes`. The bypass is scoped to exactly this self-purging delayed message, so it cannot be used to grow the stream beyond its configured limit.
    - When the schedule reads from `Nats-Schedule-Source`, the sourced content can be larger than the schedule and would break this guarantee: the server publishes the sourced content only if it still fits (the freed slot plus any remaining headroom), otherwise it falls back to the schedule's own (non-sourced) body and headers, which is guaranteed to fit.
  - **Repeating schedules (cron and `@every`)**: the schedule message must remain to keep firing, so each trigger is a net-new message. When the stream is full the generated message is rejected and dropped for that tick, exactly like any other producer hitting `DiscardNew`. The schedule message is untouched and the schedule is **not** disabled; it fires again on the next tick and succeeds once space frees up (via `MaxAge` expiry, a consumer draining the stream, or an explicit purge/delete). The server does not stall, queue, or back-fill skipped scheduled messages.

For reliable schedule replacement prefer `MaxMsgs`: a replace is always net-zero on message count, whereas under `MaxBytes` a larger replacement can still be rejected.
